...
def main(page: ft.Page):
    ...
    def dropdown_doldur(e=None):
        ...
        for ad, no, sinif in cursor.fetchall():
            ogrenci_secici.options.append(
                ft.dropdown.Option(key=str(no), text=f"{ad} ({sinif})")
            )
        page.update()
    ...
    def notlari_getir(e):
        ...
        cursor.execute(
            "SELECT * FROM notlar WHERE ogrenci_no=? ORDER BY tarih DESC",
            (ogrenci_secici.value,)
        )
    ...
    # BUTONLAR
    kaydet_btn = ft.ElevatedButton("Notu Kaydet", on_click=notu_kaydet)
    duzenle_btn = ft.ElevatedButton("Güncelle", on_click=notu_guncelle, visible=False)
